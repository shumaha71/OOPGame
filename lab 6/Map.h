...
public:
    Map(int w = 10, int h = 10)
        ...
        bases.push_back(std::make_shared<Base>("Base1", 0, 0, 100, 10));
        bases.push_back(std::make_shared<Base>("Base2", 9, 9, 100, 10));
        }
    ...
    void clear();
    const std::vector<std::shared_ptr<Base>>& getBases() const {
        return bases;
    }
